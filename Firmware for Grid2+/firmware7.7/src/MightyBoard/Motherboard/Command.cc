...
//	bool Language = eeprom::isLanguageCH();
	switch ( paused ) {
	case PAUSE_STATE_ENTER_START_PIPELINE_DRAIN:
		...
		if ( host::getBuildState() != host::BUILD_CANCELLING && host::getBuildState() != host::BUILD_CANCELED )
			Language?displayStatusMessage(PAUSE_ENTER_MSG_CH, PAUSE_DRAINING_PIPELINE_MSG_CH):displayStatusMessage(PAUSE_ENTER_MSG, PAUSE_DRAINING_PIPELINE_MSG);
		...
		paused = PAUSE_STATE_ENTER_WAIT_CLEARING_PLATFORM;
		{
		    ...
		    if (Language){
			    if ( pauseErrorMessage )
				    displayStatusMessage((sdCardError) ? CANCELLING_ENTER_MSG_CH : PAUSE_ENTER_MSG_CH,  pauseErrorMessage );
			    else
				    displayStatusMessage((cancelling) ? CANCELLING_ENTER_MSG_CH : PAUSE_ENTER_MSG_CH, PAUSE_CLEARING_BUILD_MSG_CH);
		    }else{
			    if ( pauseErrorMessage )
	...
	case PAUSE_STATE_EXIT_START_RETURNING_PLATFORM:
		...
		Language?displayStatusMessage(PAUSE_LEAVE_MSG_CH, PAUSE_RESUMING_POSITION_MSG_CH):displayStatusMessage(PAUSE_LEAVE_MSG, PAUSE_RESUMING_POSITION_MSG);
		break;
...
//	bool language = eeprom::isLanguageCH();
	...
	if( !sdcard::playbackHasNext() ) {
		...
		if (Language){
			...
			if ( sdcard::sdAvailable == sdcard::SD_ERR_NO_CARD_PRESENT ) pauseErrorMessage = NOCARD_MSG_CH;
			else if ( sdcard::sdAvailable == sdcard::SD_ERR_CRC ) pauseErrorMessage = CARDCRC_MSG_CH;
			else pauseErrorMessage = CARDERROR_MSG_CH;
		}else{
			// Establish an error message to display while cancelling the build
	...
	if ( mode == WAIT_ON_TOOL ) {
		if ( tool_wait_timeout.hasElapsed() ) {
			if (Language){
				Motherboard::getBoard().errorResponse(EXTRUDER_TIMEOUT_MSG_CH);
			}else{
	...
	if ( mode == WAIT_ON_PLATFORM ) {
		if ( tool_wait_timeout.hasElapsed() ) {
			if (Language){
				Motherboard::getBoard().errorResponse(PLATFORM_TIMEOUT_MSG_CH);
			}else{
				...
				if (command_buffer.getLength() >= 2) {
					...
					if ((axes) && (axes == (axes & ((1 << A_AXIS) | (1 << B_AXIS))))) {
                         ...
                         if (!eeprom::isSingleTool()){
                            position[0] = currentPoint[X_AXIS] - stepperAxisMMToSteps(LEVELZ_X_OFFSET_DUAL,X_AXIS);
                         }else{
                            position[0] = currentPoint[X_AXIS] - stepperAxisMMToSteps(LEVELZ_X_OFFSET_SINGLE,X_AXIS);
                         }

						if (!eeprom::isSingleTool())
							position[1] = currentPoint[Y_AXIS] - stepperAxisMMToSteps(LEVELZ_Y_OFFSET_DUAL,Y_AXIS);
						else
							position[1] = currentPoint[Y_AXIS] - stepperAxisMMToSteps(LEVELZ_Y_OFFSET_SINGLE,Y_AXIS);
				...
				if (command_buffer.getLength() >= 2) {
					...
					if ( axes == ((1 << A_AXIS) | (1 << B_AXIS)) ) {
					     ...
					     if ( alevel_valid ) {
						  // Cancel the build!
						  if ( alevel_valid == 1 )
						       pauseErrorMessage = Language?ALEVEL_INCOMPLETE_MSG_CH:ALEVEL_INCOMPLETE_MSG;
#if defined(AUTO_LEVEL_ZYYX)
						       ...
						       pauseErrorMessage = ( skew_status() == ALEVEL_COLINEAR )
							    ? (Language?ALEVEL_COLINEAR_MSG_CH:ALEVEL_COLINEAR_MSG) : (Language?ALEVEL_BADLEVEL_MSG_CH:ALEVEL_BADLEVEL_MSG);
