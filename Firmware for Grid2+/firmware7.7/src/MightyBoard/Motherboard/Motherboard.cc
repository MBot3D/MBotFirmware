...
#endif
        lcd(LCD_SCK,LCD_MOSI,LCD_DC,LCD_RST,LCD_CS),
	messageScreen(),
	mainMenu(),
	finishedPrintMenu(),
...
//	platform_thermistor(PLATFORM_PIN, TemperatureTable::table_thermistor),
	platform_thermistor(PLATFORM_PIN),
...
#else
	Extruder_One(0, EX1_PWR, EX1_FAN, THERMISTOR1_PIN,eeprom_offsets::T0_DATA_BASE),
	Extruder_Two(1, EX2_PWR, EX2_FAN, THERMISTOR2_PIN,eeprom_offsets::T1_DATA_BASE)
#endif
#ifdef PSTOP_SUPPORT
	, pstop_enabled(0)
#endif
...
//		splashScreen.hold_on = false;
		if(eeprom::isFirstTime())
		{
			interfaceBoard.pushScreen(&languageMenu);
		}	
...
#else
	cutoff.init();
	extruder_manage_timeout.start(SAMPLE_INTERVAL_MICROS_THERMISTOR);
#endif
...
//	bool Language = eeprom::isLanguageCH();
	...
	if ( user_input_timeout.hasElapsed() &&
		...
		if ( (Extruder_One.getExtruderHeater().get_set_temperature() > 0) ||
			...
			interfaceBoard.errorMessage(Language?HEATER_INACTIVITY_MSG_CH:HEATER_INACTIVITY_MSG, false);
			startButtonWait();
#ifdef HAS_RGB_LED
			// turn LEDs blue
	...
	if ( heatShutdown && !triggered && !Piezo::isPlaying() ) {
		...
		if ( heatShutdown < 3 ) {
			if ( eeprom::isSingleTool() ) msg = Language?HEATER_TOOL_MSG_CH:HEATER_TOOL_MSG;
			else if ( heatShutdown == 1 ) msg = Language?HEATER_TOOL0_MSG_CH:HEATER_TOOL0_MSG;
			else msg = Language?HEATER_TOOL1_MSG_CH:HEATER_TOOL1_MSG;
		}
		else msg = Language?HEATER_PLATFORM_MSG_CH:HEATER_PLATFORM_MSG;

		/// error message
		switch (heatFailMode) {
		case HEATER_FAIL_SOFTWARE_CUTOFF:
		        msg2 = Language?HEATER_FAIL_SOFTWARE_CUTOFF_MSG_CH:HEATER_FAIL_SOFTWARE_CUTOFF_MSG;
			break;
		case HEATER_FAIL_NOT_HEATING:
		        msg2 = Language?HEATER_FAIL_NOT_HEATING_MSG_CH:HEATER_FAIL_NOT_HEATING_MSG;
			break;
		case HEATER_FAIL_DROPPING_TEMP:
		        msg2 = Language?HEATER_FAIL_DROPPING_TEMP_MSG_CH:HEATER_FAIL_DROPPING_TEMP_MSG;
			break;
		case HEATER_FAIL_NOT_PLUGGED_IN:
			errorResponse(msg, Language?HEATER_FAIL_NOT_PLUGGED_IN_MSG_CH:HEATER_FAIL_NOT_PLUGGED_IN_MSG);
		...
		case HEATER_FAIL_BAD_READS:
			errorResponse(msg, Language?HEATER_FAIL_READ_MSG_CH:HEATER_FAIL_READ_MSG);
			heatShutdown = 0;
			return;
		default:
			break;
