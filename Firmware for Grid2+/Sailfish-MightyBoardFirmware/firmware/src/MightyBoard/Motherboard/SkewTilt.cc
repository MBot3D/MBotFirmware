...
static int32_t skew_data[3];
static float skew_dvalue;
...
static void skew_constant(void)
{
     ...
     skew_dvalue = - ( (float)(reference[0]) * (float)(skew_data[0]) + (float)(reference[1]) * (float)(skew_data[1]) +
			(float)(reference[2]) * (float)(skew_data[2]) );
}

int32_t skew(const int32_t *P)
{
     // z - ( x * Nx + y * Ny ) / Nz2
     float dvaule = - ( skew_dvalue + (float)(P[0]) * (float)(skew_data[0]) + (float)(P[1]) * (float)(skew_data[1]) ) / (float)(skew_data[2]) ;
     return ((int32_t)(dvaule));
}
