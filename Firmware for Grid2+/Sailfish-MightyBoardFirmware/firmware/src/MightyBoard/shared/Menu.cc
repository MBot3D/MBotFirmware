...
//						uint8_t left_time;
						...
						currentTemp = board.getExtruderBoard(0).getExtruderHeater().get_current_temperature();
						setTemp = board.getExtruderBoard(0).getExtruderHeater().get_set_temperature();
						lcd.setCursor(4, 6);
						lcd.writeInt(currentTemp, 3);
						lcd.write('/');
						lcd.writeInt(setTemp, 3);
						lcd.write(0x7f);
/*	
...
/*						 
						...
						if ( ++flip_flop <= 2 ) {
						 ...
						 if ( skew_active && 0 <= ( status = skew_status() ) ) {
						  lcd.moveWriteFromPgmspace(0, 6, ALEVEL_ACTIVE_MSG);
//						  lcd.setCursor(10, 6);
//						  lcd.writeFloat(stepperAxisStepsToMM(status, Z_AXIS), 3, 0);
						 }
						 else
						  lcd.moveWriteFromPgmspace(0, 6, ALEVEL_INACTIVE_MSG);
					}
