...
//	}
	...
	current_temp = TempReadtoCelsius(temp,MAX_TEMP);
	//current_temp = TemperatureTable::TempReadtoCelsius(temp,table_index,MAX_TEMP);
	return SS_OK;
}

float Thermistor::TempReadtoCelsius(int16_t adcValue,int16_t maxTemp){
	if ((adcValue ==1023)||(adcValue == 0))
		return 0.0;

	float r = RESISTANCE_VALUE / ((1023.0 / adcValue) - 1.0);

    float j = (1.0 / THERMI_B_VALUE);
	float k = (1.0 / (THERMI_T0 + 273.15));

	float celTemp = (1.0 / (k + (j * log(r / THERMI_R0)))) - 273.15;

	if (celTemp > ((float)maxTemp))
		celTemp = maxTemp;
