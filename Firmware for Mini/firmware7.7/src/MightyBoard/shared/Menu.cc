...
static void progressBar(LiquidCrystalSerial& lcd, int16_t delta, int16_t setTemp, bool blink)
{
	...
	if (lastHeatIndex == 0){
		lcd.setAddress(0, 52);
		lcd.writeBeans(heatIndex+2,31);
		lcd.setAddress(0, 53);
		lcd.writeBeans(heatIndex+2,31);
	}
	if ( lastHeatIndex > heatIndex ) {
		lcd.setAddress(0, 52);
		lcd.writeBeans(heatIndex+2,lastHeatIndex+3);
		lcd.setAddress(0, 53);
		lcd.writeBeans(heatIndex+2,lastHeatIndex+3);
	}else if (lastHeatIndex < heatIndex){
		lcd.setAddress(0, 52);
		lcd.clearBeans(lastHeatIndex+2,heatIndex+2);
		lcd.setAddress(0, 53);
		lcd.clearBeans(lastHeatIndex+2,heatIndex+2);
	}
...
void HeaterPreheatMenu::drawItem(uint8_t index, LiquidCrystalSerial& lcd) {
	...
	if (Language == CH){
		if (lind == index){
			lcd.setCursor(0,0);
			lcd.write(0x04);
			lcd.writeFromPgmspaceCH_R(PREHEATING_MSG_CH);
			for(int i=0; i<11; i++)
				lcd.write(0x04);
			return;
		}
		...
		if (lind == index){
			lcd.writeFromPgmspaceCH(preheatActive ? STOP_MSG_CH : GO_MSG_CH);
			return;
		}
		lind++;

		if (lind == index){
			msg = singleTool ? TOOL_MSG_CH : RIGHT_SPACES_MSG_CH;
			test = _rightActive;
		}
		lind++;
		...
		lcd.moveWriteFromPgmspace(13, index, test ? ON_MSG : OFF_MSG);
	}else if (Language == EN){
		if (lind == index){
			lcd.setCursor(0,0);
			lcd.write(0x04);
			lcd.writeFromPgmspace_R(PREHEATING_MSG);
			for(int i=0; i<8; i++)
				lcd.write(0x04);
			return;
		}
...
void FilamentScreen::update(LiquidCrystalSerial& lcd, bool forceRedraw) {
		...
		if ( Motherboard::getBoard().getExtruderBoard(toolID).getExtruderHeater().has_reached_target_temperature() ) {
			...
			if ( setTemp < filamentTemp[toolID] ) {
				...
				MenuBadness(Language ? EXTEMP_CHANGE_MSG_CH:EXTEMP_CHANGE_MSG);
				return;
			}
	...
	if (forceRedraw || needsRedraw) {
		...
		case FILAMENT_STOP:
			if(Language == CH)
			{
				if ((toolID == 0) && (forward == true)){
					lcd.writeFromPgmspaceCH(singleTool?LOAD_SINGLE_MSG_CH:LOAD_RIGHT_MSG_CH);
				}else if ((toolID == 0) && (forward == false)){
					lcd.writeFromPgmspaceCH(singleTool?UNLOAD_SINGLE_MSG_CH:UNLOAD_RIGHT_MSG_CH);
				}else if ((toolID == 1) && (forward == true)){
					lcd.writeFromPgmspaceCH(LOAD_LEFT_MSG_CH);
				}else if ((toolID == 1) && (forward == false)){
					lcd.writeFromPgmspaceCH(UNLOAD_LEFT_MSG_CH);
				}
				lcd.writeString("...");
				lcd.setCursor(0,3);
				lcd.write(0x01);
...
void FilamentMenu::drawItem(uint8_t index, LiquidCrystalSerial& lcd) {
	...
	if(Language == CH)
	{
		...
		if ( index == lind ){
			lcd.writeFromPgmspaceCH(PREHEATING_MSG_CH);
		}
		lind++;
	...
	else if(Language == EN)	
	{
		...
		if ( index == lind ){
			lcd.writeFromPgmspace(PREHEATING_MSG);
		}
		lind++;
...
//			lcd.moveWriteFromPgmspace(0, 6, CLEAR_MSG);
	...
	case 0:
		if ( !singleTool ) {
			...
			lcd.setCursor(1, 6);
			lcd.writeFromPgmspace(EXTRUDER1_TEMP_SMSG);
			if ( board.getExtruderBoard(0).getExtruderHeater().has_failed() || data >= BAD_TEMPERATURE )
				lcd.moveWriteFromPgmspace(7,6,NA_MSG);
			else{
				lcd.setCursor(7, 6);
				lcd.writeInt(data, 3);
			}
		...
		if ( !singleTool ) {
			if( !board.getExtruderBoard(0).getExtruderHeater().has_failed() &&
				...
				if ( data > 0 ) {
					...
					lcd.moveWriteFromPgmspace(11, 6, NA_MSG);
			}
		}
		break;
	case 2:
	...
	case 3:
	{
		...
		if (!singleTool)
			lcd.moveWriteFromPgmspace(1, 6,EXTRUDER2_TEMP_SMSG);
		if ( board.getExtruderBoard(tool).getExtruderHeater().has_failed() || data >= BAD_TEMPERATURE ){
			lcd.moveWriteFromPgmspace(singleTool ? 4 : 7, 6,NA_MSG);
		}else{
			lcd.setCursor(singleTool ? 4 : 7, 6);
			lcd.writeInt(data, 3);
		}
		break;
	}
	...
	case 4:
	{
		...
		if ( !board.getExtruderBoard(tool).getExtruderHeater().has_failed() &&
			...
			if( data > 0 ) {
				lcd.setCursor(singleTool ? 8 : 11, 6);
				lcd.writeInt(data, 3);
			}
			else
				lcd.moveWriteFromPgmspace(singleTool ? 8 : 11, 6, NA_MSG);
		}
		break;
	}
	case 5:
	...
	case 6:	
		...
		if ((state == host::HOST_STATE_BUILDING) || (state == host::HOST_STATE_BUILDING_FROM_SD) ) {
			...
			if(buildPercentage < 100){
				...
				lcd.write('%');
				progressBar(lcd, 100-buildPercentage, 100, true);
...
void PreheatSettingsMenu::resetState() {
	...
	counterLeft = eeprom::getEeprom16(eeprom_offsets::PREHEAT_SETTINGS + preheat_eeprom_offsets::PREHEAT_RIGHT_TEMP, DEFAULT_PREHEAT_TEMP);
...
void PreheatSettingsMenu::drawItem(uint8_t index, LiquidCrystalSerial& lcd) {
	...
	if (Language == CH){
		if ( index == lind ) {
			lcd.setCursor(0,0);
			lcd.write(0x04);
			lcd.writeFromPgmspaceCH_R(PREHEAT_SET_MSG_CH);
			for(int i=0; i<7; i++)
				lcd.write(0x04);
			return;
		}
		...
		if ( index == lind ) {
			lcd.setCursor(0,0);
			lcd.write(0x04);
			lcd.writeFromPgmspace_R(PREHEAT_SET_MSG);
			return;
		}
		lind++;
...
void GeneralSettingsMenu::resetState()
{
	singleExtruder = eeprom::isSingleTool();
...
void GeneralSettingsMenu::drawItem(uint8_t index, LiquidCrystalSerial& lcd)
{
	uint8_t lind = 0;
	uint8_t row = index % 7;
	...
	if(Language == CH)
	{
		if ( index == lind ) {
			lcd.setCursor(0,0);
			lcd.write(0x04);
			lcd.writeFromPgmspaceCH_R(GENERAL_SET_MSG_CH);
			for(int i=0; i<7; i++)
				lcd.write(0x04);
		}
		lind++;
		...
		if ( index == lind ) {
			...
			lcd.moveWriteFromPgmspaceCH(1, row, PAUSE_HEAT_MSG_CH);
		}
		lind++;

		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(13, row, soundOn ? ON_MSG : OFF_MSG);
			lcd.moveWriteFromPgmspaceCH(1, row, SOUND_MSG_CH);
		}
		lind++;
		...
		if ( index == lind ) {
			...
			lcd.moveWriteFromPgmspaceCH(1, row, TOOL_COUNT_MSG_CH);
		}
		lind++;

		if ( index == lind ) {
			lcd.moveWriteFromPgmspaceCH(1, row, EXIT_MSG_CH);
		}
		lind++;
	}
	else if (Language == EN)
	{
		if ( index == lind ) {
			lcd.setCursor(0,0);
			lcd.write(0x04);
			lcd.writeFromPgmspace_R(GENERAL_SET_MSG);
//			for(int i=0; i<7; i++)
//				lcd.write(0x04);
		...
		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(13, row, pauseHeatOn ? ON_MSG : OFF_MSG);
			lcd.moveWriteFromPgmspace(1, row, PAUSE_HEAT_MSG);
		}
		lind++;

		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(13, row, soundOn ? ON_MSG : OFF_MSG);
			lcd.moveWriteFromPgmspace(1, row, SOUND_MSG);
		}
		lind++;
		...
		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(1, row, EXIT_MSG);
		}
		lind++;
	}
...
void GeneralSettingsMenu::handleSelect(uint8_t index)
{
	uint8_t lind = 1;
...
void GeneralSettingsMenu::handleCounterUpdate(uint8_t index, int8_t up)
{
	uint8_t lind = 1;
...
void HomeOffsetsModeScreen::update(LiquidCrystalSerial& lcd, bool forceRedraw) {
	...
	if ( forceRedraw ) {
			...
			if(Language == CH)
			{
				lcd.writeFromPgmspaceCH_R(do_home_offsets ? HOME_OFFSETS_MSG_CH : TOOL_OFFSETS_MSG_CH);
				for(int i=0; i<5; i++)
					lcd.write(0x04);
			}
			else if(Language == EN)
			{
				lcd.writeFromPgmspace_R(do_home_offsets ? HOME_OFFSETS_MSG : TOOL_OFFSETS_MSG);
				for(int i=0; i<3; i++)
					lcd.write(0x04);
			}
...
void MaxZDiffScreen::update(LiquidCrystalSerial& lcd, bool forceRedraw) {
	if (forceRedraw) {
		...
		if (Language == CH){
			lcd.moveWriteFromPgmspaceCH_R(0, 0, ALEVEL_UTILITY_MSG_CH);
			lcd.moveWriteFromPgmspaceCH(0, 1, ALEVEL_SCREEN_MSG1_CH);
//			lcd.moveWriteFromPgmspace(0, 6, UPDNLM_MSG);
		}else if (Language == EN){
			lcd.moveWriteFromPgmspace_R(0, 0, ALEVEL_UTILITY_MSG);
			lcd.moveWriteFromPgmspace(0, 1, ALEVEL_SCREEN_MSG1);
//			lcd.moveWriteFromPgmspace(0, 6, UPDNLM_MSG);
		}
	}
...
//	case ButtonArray::LEFT:
	...
	else if ( fmax_zdelta > 4.99 ) fmax_zdelta = 4.99;
}
#endif
...
{	
			// Do we need to redraw the whole menu?
	if (/*(itemIndex/LCD_SCREEN_HEIGHT) != (lastDrawIndex/LCD_SCREEN_HEIGHT)||*/ forceRedraw || needsRedraw) {
		// Redraw the whole menu
		lcd.clear();
...
void SettingsMenu::drawItem(uint8_t index, LiquidCrystalSerial& lcd) {
	...
	if(Language == CH)
	{
		...
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspaceCH(1, row, HOME_OFFSETS_MSG_CH);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspaceCH(1, row, TOOL_OFFSETS_MSG_CH);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspaceCH(1, row, HOME_AXES_MSG_CH);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspaceCH(1, row, PLATE_LEVEL_MSG_CH);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspaceCH(1, row, JOG_MSG_CH);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspaceCH(1, row, GENERAL_SET_MSG_CH);
		}
		lind++;
		if ( index == lind ) {
		...
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspaceCH(1, row, LANGUAGE_MSG_CH);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspaceCH(1, row, EXIT_MSG_CH);
	...
	else if(Language == EN)
	{
		...
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(1, row, HOME_OFFSETS_MSG);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(1, row, TOOL_OFFSETS_MSG);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(1, row, HOME_AXES_MSG);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(1, row, PLATE_LEVEL_MSG);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(1, row, JOG_MSG);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(1, row, GENERAL_SET_MSG);
		}
		lind++;
		if ( index == lind ) {
		...
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(1, row, LANGUAGE_MSG);
		}
		lind++;
		if ( index == lind ) {
			lcd.moveWriteFromPgmspace(1, row, EXIT_MSG);
...
void SettingsMenu::handleSelect(uint8_t index) {
	...
	if ( index == lind ) {
		if (singleExtruder){
			if (Language)
				host::startOnboardBuild(utility::LEVEL_PLATE_STARTUP_SINGLE_CH);
			else
				host::startOnboardBuild(utility::LEVEL_PLATE_STARTUP_SINGLE);
		}else{
			if (Language)
				host::startOnboardBuild(utility::LEVEL_PLATE_STARTUP_DUAL_CH);
			else
				host::startOnboardBuild(utility::LEVEL_PLATE_STARTUP_DUAL);
		}
		return;
	}
	lind++;
	...
	lind++;
	if ( index == lind ) {
		interface::pushScreen(&generalsettingsMenu);
		return;
	}
	lind++;
	if ( index == lind ) {
...
void FinishedPrintMenu::drawItem(uint8_t index, LiquidCrystalSerial& lcd) {
	...
	if (Language == CH){
		switch (index) {
		case 0:
			lcd.writeFromPgmspaceCH(BUILD_FINISH_MSG_CH);
			lcd.write('!');
			break;
		case 1:
			lcd.writeFromPgmspaceCH(BUILD_TIME2_MSG_CH);
			lcd.write(':');
			break;
		case 2:
			...
			break;
		case 3:
			lcd.writeFromPgmspaceCH(FILAMENT_MSG_CH);
			lcd.write(':');
			break;
		case 4:
			printFilamentUsed(command::filamentUsed(), lcd);
			break;
		case 5:
			lcd.writeFromPgmspaceCH((lastFileIndex != 255) ? PRINT_ANOTHER_MSG_CH : CANNOT_PRINT_ANOTHER_MSG_CH);
			break;
		case 6:
			lcd.writeFromPgmspaceCH(RETURN_TO_MAIN_MSG_CH);
			break;
		}
	}else if (Language == EN){
		switch (index) {
...
badness:
	...
	MenuBadness((sdcard::sdAvailable == sdcard::SD_ERR_CRC) ? (Language? CARDCRC_MSG_CH:CARDCRC_MSG) : (Language? CARDOPENERR_MSG_CH:CARDOPENERR_MSG));
}
	...
	if ( selectable ) {
		...
		if (Language){
			if ( (sdcard::sdAvailable == sdcard::SD_ERR_DEGRADED) ||
			     (sdcard::sdErrno & SDR_ERR_COMMS) ) msg = CARDCOMMS_MSG_CH;
			else if ( sdcard::sdAvailable == sdcard::SD_SUCCESS ) msg = CARDNOFILES_MSG_CH;
			else if ( sdcard::sdAvailable == sdcard::SD_ERR_NO_CARD_PRESENT ) msg = NOCARD_MSG_CH;
			else if ( sdcard::sdAvailable == sdcard::SD_ERR_OPEN_FILESYSTEM ) msg = CARDFORMAT_MSG_CH;
			else if ( sdcard::sdAvailable == sdcard::SD_ERR_VOLUME_TOO_BIG ) msg = CARDSIZE_MSG_CH;
			else if ( sdcard::sdAvailable == sdcard::SD_ERR_CRC ) msg = CARDCRC_MSG_CH;
...
void SDMenu::handleSelect(uint8_t index) {
	...
	if ( host::getHostState() != host::HOST_STATE_READY ) {
		MenuBadness(Language?BUILDING_MSG_CH:BUILDING_MSG);
		return;
	}
	else if ( !selectable )
		return;
